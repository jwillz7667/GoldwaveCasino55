...
export default class Blackjack extends GameInterface {
  ...
  updateCardDisplay() {
    ...
    playerCards.innerHTML = this.playerHand.map(card => 
      `<div class="card ${card.suit === '♥' || card.suit === '♦' ? 'red' : 'black'}">${card.value}${card.suit}</div>`
    ).join('');
    
    // Show dealer's first card, hide second during gameplay
    if (this.gameStatus === 'playing') {
      dealerCards.innerHTML = `
        <div class="card ${this.dealerHand[0].suit === '♥' || this.dealerHand[0].suit === '♦' ? 'red' : 'black'}">
          ${this.dealerHand[0].value}${this.dealerHand[0].suit}
        </div>
        <div class="card back">?</div>
      `;
    } else {
      dealerCards.innerHTML = this.dealerHand.map(card => 
        `<div class="card ${card.suit === '♥' || card.suit === '♦' ? 'red' : 'black'}">${card.value}${card.suit}</div>`
      ).join('');
    }

    this.container.querySelector('#player-score').textContent = this.calculateHandValue(this.playerHand);
    if (this.gameStatus !== 'playing') {
      this.container.querySelector('#dealer-score').textContent = this.calculateHandValue(this.dealerHand);
    }
  }
